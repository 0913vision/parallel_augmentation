...
def main(processors:int, loaders:int, workers: int, osts:int, dups:int, image_path:str, save_path:str):
    ...
    if mpi.rank == 0:
        ...
        loader = Loader.Loader(mpi=mpi, first_worker_rank=first_worker_rank, num_workers=workers, processors=processors)
        mpi.barrier()
        loader.start()
        loader.join()
        mpi.barrier()
        ...
        worker = Worker.Worker(mpi=mpi, loader_rank=loader_rank, ost=(loader_rank-processors)%osts, save_path=save_path, dups=dups)
        mpi.barrier()
        worker.start()
        worker.join()
        mpi.barrier()
