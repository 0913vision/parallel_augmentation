...
def main(processors:int, loaders:int, workers: int, osts:int, dups:int, image_path:str, save_path:str, loader_get_ost:bool):
    ...
    if mpi.rank == 0:
        ...
        worker = Worker.Worker(mpi=mpi, loader_rank=loader_rank, ost=(loader_rank-processors)%osts, save_path=save_path, dups=dups, processors=processors, loaders=loaders)
        mpi.barrier()
        worker.start()
        worker.join()
        mpi.barrier()
